...
# CVE-2022-36883 --> Just useless noise
sudo nuclei -list "/tmp/bd_24h.txt" -templates "/root/nuclei-templates" -resolvers "/tmp/resolvers.txt" -tags "git" -exclude-id "CVE-2022-36883" -headless -disable-redirects -concurrency 100 -rate-limit 200 -project -stats -o "/tmp/nuclei_out.txt" > "/tmp/nuclei_out.log" 2>&1
# If using cent-templates
# sudo nuclei -list "/tmp/bd_24h.txt" -templates "$HOME/cent-nuclei-templates" -resolvers "/tmp/resolvers.txt" -tags "git" -exclude-id "CVE-2022-36883" -headless -disable-redirects -concurrency 100 -rate-limit 200 -project -stats -o "/tmp/nuclei_out.txt" > "/tmp/nuclei_out.log" 2>&1
#View logs: tail -f "/tmp/nuclei_out.log"
#View Output: tail -f  "/tmp/nuclei_out.txt"
#Parse & Add
#Output
 sort -u "/tmp/nuclei_out.txt" -o "/tmp/Nuclei_Git.txt"
#Logs
 cat "/tmp/nuclei_out.log" | ansi2txt | tee "/tmp/Nuclei_Git.log" 
#----------------------------------------------------------------------------#
...
#----------------------------------------------------------------------------#
#TG-BOT
 echo -e "*Job* : ⚛️ Nuclei Scan DOT Git (.git) Exposures 🪲 ➼ 🇧🇩 [bd_ccTLDs](https://github.com/RebootEx/CertStream-Bangladesh/blob/main/Data/bd_ccTLDs/certstream_domains_bd_all_24h.txt)" > "/tmp/INVENTORY_TG_BOT.md"
...
#Attachments
 ...
 cp "/tmp/Nuclei_Git.txt" "/tmp/Nuclei_Git_$SAFE_END_TIME_BST.txt"
 cp "/tmp/Nuclei_Git.log" "/tmp/Nuclei_Git_$SAFE_END_TIME_BST.log"
#Send Attachments
 apprise "tgram://$LOONIX_TG_BOT/?format=markdown" -vv -b "*Logs & Output*" --attach "/tmp/Nuclei_Git_$SAFE_END_TIME_BST.log" --attach "/tmp/Nuclei_Git_$SAFE_END_TIME_BST.txt"
#----------------------------------------------------------------------------#
